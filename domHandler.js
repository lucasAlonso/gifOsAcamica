async function showGiphyResults(jsonFound) {
    for (let index = 0; index < jsonFound.data.length; index++) {
        ...
        await wrapGiphyGif(jsonFound.data[index].id, fatherNode, "imgSearched", "imgConteiner");
    }
}
...
function showGiphyTrending(jsonFound) {
    for (let index = 0; index < jsonFound.data.length; index++) {
        ...
        wrapGiphyGif(jsonFound.data[index].id, fatherNode, "imgTrending", "imgConteinerTrendy");
    }
}
async function showGiphyRandom() {
    ...
    for (let index = 0; index < 4; index++) {
        ...
        nodeImg[index].src = "https://media.giphy.com/media/" + jsonFound.data.id + "/giphy.gif";
        nodeTitle[index].innerHTML = "#" + jsonFound.data.title.replace(/\s/g, "").slice(0, 15);
    }
}
...
function getDivWithTagsHardcodeado() {
    ...
    for (let index = 0; index < 3; index++) {
        hashtags += "#" + hashtagsHarcodeadosHorribles[getRandomArbitrary(0, hashtagsHarcodeadosHorribles.length)];
    }
...
async function setupUploadedPage() {
    ...
    newGifUrl = "https://media.giphy.com/media/" + dataReturned.data.id + "/giphy.gif";
