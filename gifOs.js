...
let jsonTrending = [];
let jsonRandom = [];
let hashtagsHarcodeadosHorribles = ['acamica:', 'no', 'se', 'hace', 'una', 'feature', 'sin', 'que', 'el', 'recurso', 'este', 'disponible', 'also', 'las', 'guias', 'son', 'muy', 'malas']
...
async function replaceRandom(nodeNumber) {
    ...
    nodeImg[nodeNumber].src = 'https://media.giphy.com/media/' + jsonFound.data.id + '/giphy.gif';
    nodeTitle[nodeNumber].innerHTML = '#' + jsonFound.data.title.replace(/\s/g, '').slice(0, 15);
}
...
async function sugestionsBarMaker() {
    if (document.getElementById("searchBarInput").value) {
        let suggestions = await getSuggestions(document.getElementById('searchBarInput').value);
        ...
        for (let index = 0; index < 3; index++) {
            if (suggestions.results[index]) {
                ...
                newDiv.addEventListener('click', () => makeSuggestionSearch(suggestions.results[index]));
