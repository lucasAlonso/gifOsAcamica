...
async function searchFromGiphy() {
  ...
  let jsonFound = await getFromServer(
    "http://api.giphy.com/v1/gifs/search?&api_key=" +
      apiKey +
      "&q=" +
      search +
      "&limit=25&offset=0&rating=R&lang=es"
  );
  closeWelcomeSection();
...
async function searchFromTenor() {
  ...
  let jsonFound = await getFromServer(
    "https://api.tenor.com/v1/search?tag=" +
      ...
      "&key=" +
      apiKeyTenor +
      "&media_filter=minimal&contentfilter=off"
  );
  closeWelcomeSection();
  cleanSearchedImgs();
...
async function getRandomFromTenor() {
  let randomEndpoint =
    "https://api.tenor.com/v1/random?tag=funny&key=" +
    apiKeyTenor +
    "&media_filter=minimal&contentfilter=off";
  let jsonFound = await getFromServer(randomEndpoint);
  return jsonFound;
...
async function getSuggestions(term) {
  let suggestionsEndpoint =
    "https://api.tenor.com/v1/search_suggestions?tag=" +
    term +
    "&key=" +
    apiKeyTenor;
